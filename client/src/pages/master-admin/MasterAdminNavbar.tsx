// components/master-admin/MasterAdminNavbar.tsx - LIGHT THEME (SAME AS SUPER ADMIN)
...
export default function MasterAdminNavbar() {
  ...
  const adminName = user?.name || 'Master Admin';
  ...
  const fetchStats = async () => {
    try {
      setLoading(true);
      const response = await masterAdminApi.get('/super-admin/dashboard/stats?period=month');
  ...
  return (
    <nav className="flex items-center justify-between px-4 md:px-6 h-16 w-full bg-white border-b border-gray-200 shadow-sm">
      ...
      <div className="flex items-center gap-3">
        ...
        >
          {showMobileMenu ? <X size={20} className="text-gray-600" /> : <Menu size={20} className="text-gray-600" />}
        </button>
      ...
      <div className="flex items-center gap-2 md:gap-3">
        ...
        <div className="hidden lg:flex items-center gap-4 mr-2">
          <div className="flex items-center gap-1 text-xs text-gray-500">
            ...
            <span>{loading ? '...' : stats.totalSuperAdmins} Super Admins</span>
          </div>
        ...
        <div className="relative">
          ...
          >
            <div className="w-8 h-8 rounded-full bg-gradient-to-br from-amber-400 to-amber-600 flex items-center justify-center text-white font-bold text-sm shadow-sm group-hover:shadow-md transition-shadow">
              {initials}
            </div>
          ...
          {showProfileMenu && (
            <>
              ...
              <div className="absolute right-0 mt-2 w-56 bg-white rounded-xl shadow-lg border border-gray-100 z-50 overflow-hidden">
                {/* User Info */}
                <div className="p-4 bg-gradient-to-r from-amber-50 to-amber-100/50 border-b border-gray-100">
                  <div className="flex items-center gap-3">
          ...
          <div className="fixed left-0 top-0 bottom-0 w-64 bg-white shadow-xl z-50 lg:hidden overflow-y-auto">
            <div className="p-4 border-b border-gray-100">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-amber-500">Master Admin</p>
                </div>
              </div>
              <div className="mt-3 grid grid-cols-2 gap-2">
                <div className="bg-amber-50 rounded-lg p-2 text-center">
                  <p className="text-xs text-amber-600 font-medium">{stats.totalSuperAdmins}</p>
                  <p className="text-[10px] text-gray-500">Super Admins</p>
                </div>
                <div className="bg-amber-50 rounded-lg p-2 text-center">
                  <p className="text-xs text-amber-600 font-medium">{stats.totalRestaurants}</p>
            ...
            <div className="py-2">
              ...
              <button
                onClick={() => {
                  setShowMobileMenu(false);
                  navigate('/master-admin/restaurants');
                }}
                className="w-full flex items-center gap-3 px-4 py-3 text-sm text-gray-600 hover:bg-gray-50"
              >
              ...
              <button
                onClick={() => {
                  setShowMobileMenu(false);
                  navigate('/master-admin/payments');
                }}
                className="w-full flex items-center gap-3 px-4 py-3 text-sm text-gray-600 hover:bg-gray-50"
              >
