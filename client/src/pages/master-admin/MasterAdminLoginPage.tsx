// pages/master-admin/MasterAdminLoginPage.tsx - REDIRECT TO DASHBOARD

import { useState, useEffect } from 'react';
import { Link, useNavigate, useLocation } from 'react-router-dom';
import { Eye, EyeOff, Loader2, ArrowLeft, Crown, AlertCircle } from 'lucide-react';
...
export default function MasterAdminLoginPage() {
  ...
  const { masterAdminLogin, isAuthenticated, isMasterAdmin, isLoading: authLoading } = useAuth();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = await masterAdminLogin(email, password);
    ...
    <div style={{ minHeight: '100vh', display: 'flex', fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <div style={{ width: '45%', minWidth: 360, background: '#fff', display: 'flex', flexDirection: 'column', padding: '40px 56px', overflowY: 'auto' }}>
        ...
        <div style={{ marginBottom: 28 }}>
          ...
          <p style={{ fontSize: 13, color: '#a8a29e', marginTop: 6 }}>Sign in to manage your platform</p>
        </div>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 20, flex: 1 }}>
          ...
          <div>
            ...
            <input
              ...
              placeholder="masteradmin@apos.com"
              required
              autoFocus
              style={{
                width: '100%',
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            ...
            <Link to="/master-admin/forgot-password" style={{ fontSize: 12, color: '#1a1a2e', fontWeight: 600, textDecoration: 'none' }}>
              Forgot password?
            </Link>
          </div>
          ...
          <div style={{ textAlign: 'center', marginTop: 12 }}>
            <Link to="/super-admin/login" style={{ fontSize: 13, color: '#78716c', textDecoration: 'none', transition: 'color 0.2s' }}>
              🛡️ Super Admin Login
            </Link>
          </div>
          ...
          <p style={{ textAlign: 'center', fontSize: 13, color: '#78716c', marginTop: 4 }}>
            ...
            <Link to="/master-admin/register" style={{ color: '#1a1a2e', fontWeight: 700, textDecoration: 'none' }}>
              Create Account
            </Link>
          </p>
      ...
      <div style={{ flex: 1, background: 'linear-gradient(135deg, #1a1a2e, #16213e, #0f3460)', position: 'relative', overflow: 'hidden' }}>
        ...
        {FOOD_IMAGES.map((img, i) => (
          <div key={i} style={{
            ...
            <img
              ...
              onError={e => {
                (e.target as HTMLImageElement).style.background = '#16213e';
              }}
            />
          </div>
        ))}
        ...
        {/* Decorative Quote */}
        <div style={{ position: 'absolute', bottom: '4%', left: '50%', transform: 'translateX(-50%)', zIndex: 2, textAlign: 'center', padding: '0 40px' }}>
          <p style={{ color: 'rgba(255,255,255,0.7)', fontSize: 13, maxWidth: 380, lineHeight: 1.6 }}>
            "Welcome back, Master Admin. Manage your platform with ease and control."
          </p>
        </div>
      </div>
