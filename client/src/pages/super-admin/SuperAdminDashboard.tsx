// pages/super-admin/SuperAdminDashboard.tsx - BRANCH FILTER WITH MONTHLY GRAPH
...
export default function SuperAdminDashboard() {
  ...
  // ✅ Branch change pe dashboard refresh
  useEffect(() => {
    console.log('🔄 Branch changed, fetching dashboard data...');
    fetchDashboardData();
  }, [selectedBranch]); // Sirf branch change pe call ho

  // ✅ Range change pe dashboard refresh
  useEffect(() => {
  ...
  const generateChartData = useCallback((dailyData: any[]) => {
    ...
    // ─── YEAR: Show monthly totals (Jan, Feb, Mar...) ────────────────────
    if (selectedRange === 'year') {
    ...
    // ─── MONTH: Show ONLY the month name with TOTAL ──────────────────────
    else if (selectedRange === 'month') {
  ...
  const fetchDashboardData = useCallback(async () => {
    ...
    try {
      ...
      let statsData = null;
      try {
        const statsRes = await superAdminApi.get(`/super-admin/dashboard/stats?${params}`);
      ...
      try {
        const chartRes = await superAdminApi.get(`/super-admin/revenue/overview?${params}`);
      ...
      try {
        const paymentRes = await superAdminApi.get(`/super-admin/payments?limit=10&page=1${params.toString() ? '&' + params.toString() : ''}`);
      ...
      // ─── Fetch Recent Orders ─────────────────────────────────────────
      try {
        const orderRes = await superAdminApi.get(`/super-admin/orders/recent?limit=10${params.toString() ? '&' + params.toString() : ''}`);
      ...
      // ─── Fetch Recent Staff ──────────────────────────────────────────
      try {
        const staffRes = await superAdminApi.get(`/super-admin/staff/recent?limit=6${params.toString() ? '&' + params.toString() : ''}`);
  ...
  const handleViewAllPayments = () => {
    ...
    navigate(`/super-admin/payments?${params.toString()}`);
  };
  ...
  const handleViewAllOrders = () => {
    ...
    navigate(`/super-admin/orders?${params.toString()}`);
  };
  ...
  const handleViewAllStaff = () => {
    ...
    navigate(`/super-admin/staff?${params.toString()}`);
  };
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500">
                  Welcome back, <span className="font-medium text-gray-700">{admin?.firstName || 'Super Admin'}</span>! 
          ...
          <div className="flex items-center gap-3 w-full md:w-auto flex-wrap">
            ...
            <div className="relative">
              <select
                ...
                className="px-3 py-1.5 text-sm border border-gray-200 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent bg-white min-w-[200px] appearance-none"
                style={{
