// pages/super-admin/SuperAdminAddIngredient.tsx - FIXED BACK NAVIGATION
...
import { 
  ...
  FolderPlus, Ruler, Building2, Plus, Shield
} from 'lucide-react';
...
export default function SuperAdminAddIngredient() {
  ...
  const loadSavedForm = () => {
    try {
      const saved = localStorage.getItem('superAdminIngredientFormData');
      if (saved) {
        const parsed = JSON.parse(saved);
        return parsed;
      }
    } catch (e) {
      console.error('Error loading saved ingredient form:', e);
    }
    return {
  ...
  useEffect(() => {
    try {
      localStorage.setItem('superAdminIngredientFormData', JSON.stringify(form));
    } catch (e) {
      console.error('Error saving ingredient form:', e);
    }
  }, [form]);
    ...
    const fetchData = async () => {
      try {
        const [categoriesRes, suppliersRes, unitsRes] = await Promise.all([
          superAdminApi.get('/ingredient-categories?limit=100'),
          superAdminApi.get('/suppliers?status=active&limit=100'),
          superAdminApi.get('/units?limit=100')
        ]);
  ...
  const reset = () => {
    ...
    localStorage.removeItem('superAdminIngredientFormData');
    setError('');
    setNameErr('');
    setSkuErr('');
    setCategoryErr('');
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res = await superAdminApi.post('/super-admin/ingredients', payload);
      if (res.data.success) {
        localStorage.removeItem('superAdminIngredientFormData');
        toast.success('Ingredient created successfully!');
        navigate('/super-admin/ingredients', { replace: true });
      }
    } catch (err: any) {
  ...
  const handleAddCategory = () => {
    localStorage.setItem('superAdminIngredientFormData', JSON.stringify(form));
    // ✅ Pass returnTo parameter to navigate back to ingredient page
    navigate('/super-admin/add-ingredient-category?returnTo=ingredient');
  };

  const handleAddUnit = () => {
    localStorage.setItem('superAdminIngredientFormData', JSON.stringify(form));
    navigate('/super-admin/add-unit?returnTo=ingredient');
  };

  const handleAddSupplier = () => {
    localStorage.setItem('superAdminIngredientFormData', JSON.stringify(form));
    navigate('/super-admin/suppliers/new?returnTo=ingredient');
  };

  // ✅ FIXED: Handle back navigation based on returnTo parameter
  const handleBack = () => {
    localStorage.removeItem('superAdminIngredientFormData');
    
    // If we came from dishes/new, go back there
    if (returnTo === 'dish' || returnTo === 'dishes') {
      navigate('/super-admin/dishes/new');
    } 
    // If we came from categories, go back there
    else if (returnTo === 'category' || returnTo === 'categories') {
      navigate('/super-admin/categories');
    }
    // If we came from units, go back there
    else if (returnTo === 'unit' || returnTo === 'units') {
      navigate('/super-admin/units');
    }
    // If we came from suppliers, go back there
    else if (returnTo === 'supplier' || returnTo === 'suppliers') {
      navigate('/super-admin/suppliers');
    }
    // Default: go to ingredients list
    else {
      navigate('/super-admin/ingredients', { replace: true });
    }
  };
  ...
  const handleCancel = () => {
    ...
    // Same logic as handleBack for cancel
    if (returnTo === 'dish' || returnTo === 'dishes') {
      navigate('/super-admin/dishes/new');
    } else if (returnTo === 'category' || returnTo === 'categories') {
      navigate('/super-admin/categories');
    ...
    <div style={{ minHeight: '100vh', background: '#faf9f7', fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <div style={{ padding: '0 24px 40px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
            ...
            <div>
              <h1 style={{ fontSize: 16, fontWeight: 700, color: '#111827', margin: 0 }}>
                {returnTo === 'dish' || returnTo === 'dishes' ? 'Add Ingredient for Dish' : 'Create Ingredient'}
              </h1>
              <p style={{ fontSize: 12, color: '#9ca3af', margin: 0 }}>
                {returnTo === 'dish' || returnTo === 'dishes' 
                  ? 'Add raw material to inventory for your dish' 
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          {/* Stock Information */}
          <Section icon={Beef} iconColor="#8b5cf6" title="Stock Information">
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
                <div>
          ...
          <Section icon={Truck} iconColor="#8b5cf6" title="Supplier Information">
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              <div>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
          ...
          {/* Status */}
          <Section icon={Sparkles} iconColor="#8b5cf6" title="Status">
