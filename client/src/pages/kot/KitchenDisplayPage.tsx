...
export default function KitchenDisplayPage() {
  ...
  const fetchOrders = useCallback(async () => {
    try {
      // Get the staff token from localStorage
      ...
      if (res.data.success) {
        ...
        if (soundEnabled && kitchen.length > lastCount && lastCount > 0) {
          // Play sound (using Web Audio API as fallback)
          try {
            const audio = new Audio();
            // Use a simple beep via Web Audio if needed
  ...
  }, [lastCount, soundEnabled, navigate]);
