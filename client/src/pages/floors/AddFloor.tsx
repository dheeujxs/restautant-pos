...
export default function AddFloorPage() {
  ...
  const handleSubmit = async () => {
    ...
    setNameErr('');
    try {
      const res = await adminApi.post('/floors', { name, description, displayOrder, isActive });
      if (res.data.success) {
        navigate('/floors');
      }
    } catch (err: any) {
      ...
      <div style={{ padding: '0 24px 40px' }}>
        ...
        <div style={{ height: 56, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
            ...
            <div>
              <h1 style={{ fontSize: 16, fontWeight: 700, color: '#111827', margin: 0 }}>Create Floor</h1>
              <p style={{ fontSize: 12, color: '#9ca3af', margin: 0 }}>Add new restaurant floor</p>
            </div>
          </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10, paddingTop: 4 }}>
            ...
            >
              {loading ? <><Loader2 size={14} style={{ animation: 'spin 1s linear infinite' }} /> Creating…</> : <><Sparkles size={14} /> Save Floor</>}
            </button>
          </div>
