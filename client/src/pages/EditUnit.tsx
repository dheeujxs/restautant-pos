...
import { 
  ...
  Package, Hash, AlignLeft, Tag, Edit3
} from 'lucide-react';
...
export default function EditUnitPage() {
  ...
  useEffect(() => {
    const fetchUnit = async () => {
      try {
        const res = await adminApi.get(`/units/${id}`);
        if (res.data.success) {
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      const res = await adminApi.patch(`/units/${id}`, form);
      if (res.data.success) {
      ...
      setErrors({ submit: err.response?.data?.error || 'Failed to update unit' });
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <div style={{ minHeight: '100vh', background: '#faf9f7', fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <div style={{ padding: '0 24px', height: 56, background: 'white', borderBottom: '1px solid #f3f4f6', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
          ...
          <div>
            <h1 style={{ fontSize: 16, fontWeight: 700, color: '#111827', margin: 0 }}>Edit Unit</h1>
            <p style={{ fontSize: 12, color: '#9ca3af', margin: 0 }}>Update measurement unit details</p>
          </div>
        </div>
      </div>
      ...
      <div style={{ padding: '24px 24px 40px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 12, paddingTop: 4 }}>
            ...
            >
              {submitting ? <><Loader2 size={16} style={{ animation: 'spin 1s linear infinite' }} /> Saving…</> : <><Edit3 size={16} /> Update Unit</>}
            </button>
          </div>
